...
class KMP:
    ...
    def __process_container(self, url: str, root: str, task_list:Queue|None) -> Queue:
        ...
        while not reqs:
            try:
                ...
                reqs = fetch_dynamic_content(url, timeout=20, headers=request_headers, root_selector="main#main") # 我修改的代码
            except requests.exceptions.Timeout:
                logging.warning("Connection timed out, this may be due to CAPTCHA, please open Kemono and solve the captcha, program will sleep for 20 seconds")
                time.sleep(20)
            except(requests.exceptions.RequestException) as e:
        ...
        while "500 Internal Server Error" in soup.find("title"):
            ...
            while not reqs:
                try:
                    # reqs = self.__session.get(url, timeout=10, headers=request_headers)
                    reqs = fetch_dynamic_content(url, timeout=20, headers=request_headers, root_selector="a.fileThumb") # 我修改的代码
                except requests.exceptions.Timeout:
                    logging.warning("Connection timed out, this may be due to CAPTCHA, please open Kemono and solve the captcha, program will sleep for 20 seconds")
                    time.sleep(20)
                except(requests.exceptions.RequestException) as e:
    ...
    def __process_window(self, url: str, continuous: bool, get_list:bool=False, pool:ThreadPool|None=None, stop_url:str = None, override_path:str = None) -> Queue:
        ...
        while not reqs:
            try:
                # reqs = self.__session.get(url, timeout=10, headers=request_headers)
                reqs = fetch_dynamic_content(url, timeout=20, headers=request_headers, root_selector="meta[name='artist_name']") # 我修改的代码
            except requests.exceptions.Timeout:
                logging.warning("Connection timed out, this may be due to CAPTCHA, please open Kemono and solve the captcha, program will sleep for 20 seconds")
                time.sleep(20)
            except(requests.exceptions.RequestException) as e:
            ...
            if continuous:
                ...
                while not reqs:
                    try:
                        ...
                        reqs = fetch_dynamic_content(url + suffix + str(counter), timeout=10, headers=request_headers, root_selector="a[href*='/post/']") # 我修改的代码
                    except requests.exceptions.Timeout:
                        logging.warning("Connection timed out, this may be due to CAPTCHA, please open Kemono and solve the captcha, program will sleep for 20 seconds")
                        time.sleep(20)
                    except(requests.exceptions.RequestException) as e:
    ...
    def __call_and_interpret_url(self, url: str, get_list:bool=False) -> Queue|None:
        ...
        # Single artist work requires a directory similar to one if it were a window to be created, once done, it can be processed
            ...
            while not reqs:
                try:
                    # reqs = self.__session.get(url, timeout=10, headers=request_headers)
                    reqs = fetch_dynamic_content(url, timeout=20, headers=request_headers, root_selector="a.post__user-name") # 我修改的代码
                except requests.exceptions.Timeout:
                    logging.warning("Connection timed out, this may be due to CAPTCHA, please open Kemono and solve the captcha, program will sleep for 20 seconds")
                    time.sleep(20)
                except(requests.exceptions.RequestException) as e:
